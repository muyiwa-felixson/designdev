...
import { core } from "../utility/token";
import {Grider, Liner, ColorBox, FontBox, FontSize, ElevationBox, PageGrid, } from "./documentation.style";
...
const RenderSubSections = ({ data }) => {
  ...
  return Object.keys(data).map((elem) => {
    if (getObjectDepth(data[elem]) === 2) {
      return (
        <Grider>
          <Text type="title.small">{elem}</Text>
          <div>
...
const [inView, setInView] = useState(0);
  return (
    <PageGrid>
        ...
        {Object.keys(core.core).map((elem, index) => {
        return (
            <div key={`menu_${elem}`} className={index === inView ? "active" : ""}  onClick={()=> setInView(index)}>
      ...
      {Object.keys(core.core).map((elem,index) => {
        return (
          <div  key={`container_${elem}`} className={`inview ${index === inView ? "active" : ""}`} >
